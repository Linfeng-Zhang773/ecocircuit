...
const ITEMS = [
  { id: "gpu", name: "Retired GPU", bin: "recycle" },
  { id: "tpu", name: "AI accelerator", bin: "recycle" },
  { id: "battery", name: "Backup Li-ion pack", bin: "hazard" },
  { id: "psu", name: "Server power supply", bin: "hazard" },
  { id: "ssd", name: "Data-center SSD", bin: "recycle" },
  { id: "nic", name: "Network switch card", bin: "recycle" },
  { id: "cooling", name: "Cooling controller", bin: "hazard" },
  { id: "board", name: "Server motherboard", bin: "recycle" },
];
...
const ITEM_DEFS: Record<string, string> = {
  gpu: "The main training hardware for many AI clusters. GPUs contain recoverable gold, copper, silver, and rare earth materials, but rapid performance cycles can retire thousands at once.",
  tpu: "A specialized AI accelerator like those measured in lifecycle studies. Certified recyclers can recover metals from chips, packages, heat spreaders, and attached memory modules.",
  battery: "Backup batteries keep servers online during outages. Li-ion packs contain cobalt, lithium, nickel, manganese, flammable electrolytes, and toxic metals requiring specialized hazardous-waste handling.",
  psu: "Server power supplies contain copper, aluminum, capacitors, solder, flame retardants, and sometimes hazardous residues. They should be handled through certified electronics recyclers, not landfill streams.",
  ssd: "AI data centers use large storage arrays for models, logs, and training data. SSDs contain circuit boards, controllers, NAND packages, gold contacts, and recoverable metals.",
  nic: "High-speed networking cards connect GPU and TPU clusters. Their circuit boards contain copper traces, gold connectors, solder, and flame retardants that require formal recycling.",
  cooling: "Cooling electronics regulate fans, pumps, sensors, and liquid-cooling loops. Controllers can contain lead solder, brominated flame retardants, and residues from cooling systems.",
  board: "Server motherboards hold CPUs, memory slots, voltage regulators, connectors, and dense copper traces. They are valuable e-waste but require certified handling to avoid toxic exposure.",
};

const ITEM_EXPLANATIONS: Record<string, string> = {
  gpu: "Retired GPUs belong in certified e-waste recycling because their boards and packages contain recoverable metals. The AI-specific issue is scale: one cluster refresh can discard thousands of working but no-longer-competitive units.",
  tpu: "AI accelerators should be formally recycled so metals, substrates, memory modules, and heat spreaders can be recovered. They should not enter informal recycling streams where open burning and acid stripping expose workers to toxins.",
  battery: "Backup Li-ion packs are hazardous waste. Their flammable electrolytes and cobalt, lithium, nickel, and manganese content create fire and contamination risks when dumped or processed informally.",
  psu: "Server power supplies are treated as hazardous electronics because capacitors, solder, flame retardants, and dense metal assemblies require careful handling before material recovery.",
  ssd: "Data-center SSDs are recyclable through certified facilities. They contain valuable metals and circuit boards, and secure destruction can be paired with responsible material recovery instead of disposal.",
  nic: "Network switch cards are recyclable e-waste. High-speed AI interconnects add to the hardware footprint because training clusters need dense networking, not just GPUs or TPUs.",
  cooling: "Cooling controllers should be handled as hazardous electronics when they contain lead solder, flame retardants, or residues from liquid-cooling systems. Cooling infrastructure is part of the AI hardware lifecycle too.",
  board: "Server motherboards are recyclable but toxic if processed informally. Certified recyclers can recover gold, silver, copper, and other metals while controlling exposure to lead solder and brominated flame retardants.",
};
...
export function EWaste() {
  ...
  return (
    <SectionShell
      ...
      description={<>62 million tons of <GlossaryTerm term="e-waste" definition="Electronic waste, discarded devices, components, and infrastructure. For AI, this includes GPUs, TPUs, storage drives, networking equipment, power supplies, cooling electronics, backup batteries, and server boards. Only 22.3% is formally recycled globally." /> leave our pockets and racks every year. Less than a quarter is formally recycled, the rest leaks heavy metals into soil and water.</>}
    >
        ...
        <div className="lg:col-span-3 glass rounded-3xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Sort the hardware from an AI data-center refresh. Hover names to learn more.</p>
            </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {sorted[b.id].map((id) => {
                      ...
                      return (
                        ...
                        >
                          {it?.name} {isDone ? (right ? "correct" : "review") : ""}
                        </span>
                      );
                    })}
                  </div>
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2 font-mono font-semibold mb-1">
                      <span className={correct ? "text-neon" : "text-destructive-foreground"}>
                        {correct ? "Correct:" : "Review:"} {it.name} to {binLabel}
                      </span>
                      {!correct && placedInBin && (
